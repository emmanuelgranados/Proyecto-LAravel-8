$(function () {
    ...
    $('#fk_id_categorias_tareas.select2').select2({

        theme: "classic",
        dropdownParent: $('#add-tarea')
    });

    $('#fk_id_categorias_tareased.select2').select2({
    ...
    $("#formNuevaTareaPredefinida").on("submit", function(e){
        ...
        Swal.fire({
            ...
            $.ajax({
            type:'POST',
            url:'new_tareas_predefinidas',
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            data:datos,
            success:function(data){
                limpiar();
                tabla_tareas();
                $('#cerrarModalNuevaTareaPredefinida').trigger("click");
                Swal.fire("¡Éxito!", "Se agrego un nuevo registro.", "success");
            },
            error: function(response) {
                $('#agregar_tarea_predefinida').trigger("click");
...
function tabla_tareas(){
    ...
    $('#tareas_predefinidas').DataTable({
        searchDelay: 400,
        ajax: {
            url: 'api/lista_tareas_predefinidas',
        ...
        columns:[
            ...
            {data:'categoria', defaultContent: "---", title: "Categoria"},
...
console.log(id);
$("#id_tareaPredeterminada").val(id);
$.ajax({
    type:'POST',
    url:'api/detalle_tareas_predefinidas',
    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
    data:{id:id},
    success:function(data){
       console.log(data);
       $('#tarea_predefinidaed').val(data[0].tarea_predefinida);
       $('#fk_id_categorias_tareased').val(data[0].idcategoria).change();
          }
 });

 $("#formEditarTareaPredefinida").on("submit", function(e){
    ...
    Swal.fire({
        ...
        success:function(data){
            ...
            $('#cerrarModalEditarTareaPredefinida').trigger("click");
...
function delete_tarea(id){
    ...
    if (result.value) {
        ...
        $.ajax({
            type:'POST',
            url:'delete_tareas_predefinidas',
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            data:{id:id},
            success:function(data){
...
function bloquear(id){
    ...
    if (result.value) {
        ...
        $.ajax({
            type:'POST',
            url:'bloquear_tareas_predefinidas',
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            data:{id:id},
            success:function(data){
                tabla_tareas();
...
function desbloquear(id){
    ...
    if (result.value) {
        ...
        $.ajax({
            type:'POST',
            url:'desbloquear_tareas_predefinidas',
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            data:{id:id},
            success:function(data){
                tabla_tareas();
