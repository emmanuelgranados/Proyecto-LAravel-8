...
Route::get('/detalle_tarea', function (Request $request) {
    ...
    $tarea = Tareas::with('clientes','prioridades','estatus','comentarios.usuarios','usuariosAlta','usuariosAsignado','archivos','seguimiento.acciones','seguimiento.usuariosAlta','seguimiento.usuariosAsignado','seguimiento.usuarios')
    ->where('id',$request->id)
    ->first();
...
Route::get('/obtener_lista_tareas_pendientes', function (Request $request ) {

    $tareas = Tareas::with('clientes','prioridades','usuariosAlta','usuariosAsignado','estatus')
        ->orderBy('id','DESC')
        ->where('fk_id_estatus',2)
        ->where('fk_id_users_asignado',$request->fk_id_users)
        ->where('eliminado',0)
        ->get();
